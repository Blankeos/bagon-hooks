...
import { useScrollSpy } from 'bagon-hooks';
          ...
          <For each={spy.data()}>
            {(heading, index) => (
              <button
                ...
                onClick={() =>
                  heading.getNode().scrollIntoView({ block: 'start', behavior: 'smooth' })
                }
              >
                {heading.value}
              </button>
        ...
        <div ref={setScrollRef} class="spy-demo h-full flex-1 space-y-6 overflow-y-auto p-4">
          ...
          <section>
            ...
            <p class="text-neutral-600">
              ...
              Clicking a nav item scrolls within the card via `scrollIntoView`.
            </p>
          </section>
          <section>
