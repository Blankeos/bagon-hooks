// @ts-nocheck — faithful upstream port; noUncheckedIndexedAccess noise on mask slot indexing
...
export function useMask(options: UseMaskOptions): UseMaskReturnValue {
  ...
  const processedRef = { current: '' };
  const displayValueRef = { current: '' };
  const rawValueRef = { current: '' };
  const wasCompleteRef = { current: false };
  const isFocusedRef = { current: false };
  const undoStackRef = { current: [] as UndoState[] };
  const redoStackRef = { current: [] as UndoState[] };
