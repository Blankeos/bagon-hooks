// @ts-nocheck — faithful upstream port; noUncheckedIndexedAccess noise across panel/size math
import { Accessor, JSX, createEffect, createMemo, createSignal, onCleanup } from 'solid-js';
import { useUncontrolled } from '../use-uncontrolled/use-uncontrolled';

type RefCallback<T> = (element: T | null) => void;
type KeyboardEventHandler = (event: KeyboardEvent) => void;
type MouseEventHandler = (event: MouseEvent | TouchEvent) => void;
...
export interface UseSplitterReturnValue<T extends HTMLElement = any> {
  ...
  sizes: Accessor<SplitterPaneSize[]>;
...
export function useSplitter<T extends HTMLElement = any>(
  ...
  const [activeHandle, setActiveHandle] = createSignal(-1);
  const [containerSize, setContainerSize] = createSignal(0);

  const optionsRef = { current: options };
  optionsRef.current = options;

  const internalStateRef = { current: createInitialInternalState() as SplitterInternalState };
  const containerRef = { current: null as T | null };
  const [containerNode, setContainerNode] = createSignal<T | null>(null);
  const containerSizeRef = { current: 0 };
  const rootFontSizeRef = { current: 16 };
  const documentControllerRef = { current: null as AbortController | null };
  const frameRef = { current: 0 };
  const currentSizesRef = { current: currentSizes() as SplitterPaneSize[] };
  createEffect(() => {
    currentSizesRef.current = currentSizes();
  });

  const preCollapseSizesRef = { current: defaultSizes as SplitterPaneSize[] };

  const collapsed = createMemo(() => currentSizes().map((size) => sizeMagnitude(size) === 0));
  ...
  const containerRefCallback: RefCallback<T | null> = ((node) => {
  ...
  const handleRefCallbacks = { current: new Map<number, (node: HTMLElement | null) => void>() };
  const handleElementControllers = { current: new Map<number, AbortController>() };

  const getHandleRefCallback = (
    (handleIndex: number): RefCallback<HTMLElement> => {
